# 🧑🏻‍💻 Marcos Paulo

**`Desenvolvedor Backend`**

Me chamo **Marcos Paulo Vimieiro**, formado em Ciência da Computação pela UNIFAGOC. Apaixonado por tecnologia e em constante busca por aprofundamento, curso Pós-graduação Lato Sensu em Engenharia de Software pela PUC Minas, onde consolido uma base sólida em arquitetura e práticas de desenvolvimento. Minha área de especialização é o Backend com foco em **Node** e **NestJS**, sou entusiasta de ferramentas como Docker e bancos de dados como PostgreSQL, MySQL e MongoDB.

---

### 💻 Habilidades Técnicas

Minhas principais tecnologias incluem:

- 🛠️ **Backend:** Node, Nest, Java, Spring, Python
- 🗄️ **Banco de Dados:** PostgreSQL, MySQL, Oracle, MongoDB
- 💡 **Outras:** Docker, AWS, React

---

### 🎓 Formação Acadêmica

- **Pós-graduação Lato Sensu em Engenharia de Software** - PUC Minas (04/2025 - 04/2026)
- **Ciência da Computação** - UNIFAGOC (01/2021 - 12/2024)

---

### 📊 Estatísticas

<p>
  <img 
    align="left" 
    alt="GitHub Stats" 
    height="200" 
    style="padding-right: 10px;" 
    src="https://github-readme-stats.vercel.app/api?username=marcospaulov&show_icons=true&theme=tokyonight&include_all_commits=true&locale=pt-br" 
  />

<img 
      align="left" 
      alt="GitHub Stats" 
      height="200" 
      src="https://github-readme-stats.vercel.app/api/top-langs/?username=marcospaulov&theme=tokyonight&layout=compact&custom_title=Tecnologias&langs_count=9" 
  />

</p>

---

### 📬 Conecte-se Comigo

---

<div>
  <!--Instagram-->
  <a href="https://instagram.com/marcospaulov0" target="_blank"><img height="30" src="https://img.shields.io/badge/-Instagram-%23E4405F?style=for-the-badge&logo=instagram&logoColor=white" alt="instagram" target="_blank"></a>
  <!-- Linkedin-->
  <a href="https://www.linkedin.com/in/marcospaulosv/" target="_blank"><img height="30" src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="linkedin" target="_blank"></a>
</div>

---
